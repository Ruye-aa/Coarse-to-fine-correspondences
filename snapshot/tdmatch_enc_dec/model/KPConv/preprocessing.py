...
def collate_fn_descriptor(list_data, config, neighborhood_limits):
    ...
    # Starting radius of convolutions开始进行卷积的半径
...
def calibrate_neighbors(dataset, config, collate_fn, keep_ratio=0.8, samples_threshold=2000):
    ...
    # From config parameter, compute higher bound of neighbors number in a neighborhood 计算邻居数的边界
    hist_n = int(np.ceil(4 / 3 * np.pi * (config.deform_radius + 1) ** 3)) #以deform_radius为半径的球体内
    neighb_hists = np.zeros((config.num_layers, hist_n), dtype=np.int32)

    # Get histogram of neighborhood sizes i in 1 epoch max.   在一个epoch max中获取邻域i的直方图
